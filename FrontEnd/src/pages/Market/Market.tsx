...
function Market() {
  ...
  const searchResults = location.state?.searchResults; // ✅ Search results passed from SearchBar
  ...
  // ✅ Updated useEffect to handle searchResults
  useEffect(() => {
    if (searchResults) {
      ...
      // Otherwise, fetch all products as before
      setLoading(true);
      api
        .get(`/products`)
        .then((response) => {
  ...
  return (
    <div className="p-4 md:p-10 flex flex-col gap-10">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {/* ✅ Updated: Show 'No products found' if filteredProducts is empty */}
        ...
        {/* Popular Products */}
        <p className="text-2xl md:text-3xl font-bold text-secondary mt-10">
          Popular Products
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredProducts.length === 0 ? (
